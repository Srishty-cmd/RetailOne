...
export const getReportData = async (req: AuthRequest, res: Response, next: NextFunction): Promise<any> => {
  try {
    let start = new Date();
    let end = new Date();
    ...
    const lowStockProducts = await Inventory.countDocuments({ status: 'Low Stock' });
    ...
    const monthlySalesTrendMap = new Map<string, { revenue: number; ordersCount: number }>();
    monthlySalesTrend.forEach((item: any) => {
    ...
    // 7. Low Stock Report
    const lowStockReport = await Inventory.find({
      status: { $in: ['Low Stock', 'Out of Stock'] }
    })
      .populate('product', 'productName sku category sellingPrice')
      .lean();

    const lowStockList = lowStockReport.map((inv: any) => ({
      inventoryId: inv._id,
    ...
    const totalLowStockCount = await Inventory.countDocuments({ status: 'Low Stock' });
    const totalOutOfStockCount = await Inventory.countDocuments({ currentStock: 0 });
    const totalInStockCount = await Inventory.countDocuments({ status: 'In Stock' });
